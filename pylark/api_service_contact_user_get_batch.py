...
class BatchGetUserReq(object):
    employee_ids: typing.List[str] = attr.ib(
        factory=lambda: [], metadata={"req_type": "query", "key": "employee_ids"}
    )  # 支持通过 open_id 或者 employee_id 查询用户信息，不支持混合两种 ID 进行查询，单次请求支持的最大用户数量为100
    open_ids: typing.List[str] = attr.ib(
        factory=lambda: [], metadata={"req_type": "query", "key": "open_ids"}
    )  # 支持通过 open_id 或者 employee_id 查询用户信息，不支持混合两种 ID 进行查询，单次请求支持的最大用户数量为100
...
class BatchGetUserRespUserInfo(object):
    ...
    name_py: str = attr.ib(
        default="", metadata={"req_type": "json", "key": "name_py"}
    )  # 用户名拼音
    en_name: str = attr.ib(
        default="", metadata={"req_type": "json", "key": "en_name"}
    )  # 英文名
    employee_id: str = attr.ib(
        default="", metadata={"req_type": "json", "key": "employee_id"}
    ...
    leader_employee_id: str = attr.ib(
        default="", metadata={"req_type": "json", "key": "leader_employee_id"}
    )  # 用户直接领导的 employee_id，企业自建应用返回，应用商店应用申请了 employee_id 权限时才返回
    leader_open_id: str = attr.ib(
        default="", metadata={"req_type": "json", "key": "leader_open_id"}
    )  # 用户直接领导的 open_id
    leader_union_id: str = attr.ib(
        default="", metadata={"req_type": "json", "key": "leader_union_id"}
    )  # 用户直接领导的 union_id
    ...
    open_departments: typing.List[str] = attr.ib(
        factory=lambda: [], metadata={"req_type": "json", "key": "open_departments"}
    )  # 用户所在部门 openID 列表，用户可能同时存在于多个部门
    custom_attrs: typing.Dict = attr.ib(
        default=None, metadata={"req_type": "json", "key": "custom_attrs"}
...
class BatchGetUserResp(object):
    user_infos: typing.List[BatchGetUserRespUserInfo] = attr.ib(
        factory=lambda: [], metadata={"req_type": "json", "key": "user_infos"}
    )  # 用户信息
