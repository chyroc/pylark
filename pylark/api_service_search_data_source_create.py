...
@attr.s
class CreateSearchDataSourceReq(object):
    name: str = attr.ib(
        default="", metadata={"req_type": "json"}
    )  # data_source的展示名称, 示例值："客服工单"
    state: int = attr.ib(
        default=0, metadata={"req_type": "json"}
    )  # 数据源状态，0-未上线，1-已上线, 示例值：0, 可选值有: `0`：未上线, `1`：已上线
    description: str = attr.ib(
        default="", metadata={"req_type": "json"}
    )  # 对于数据源的描述, 示例值："搜索客服工单数据"


@attr.s
class CreateSearchDataSourceRespDataSource(object):
...
@attr.s
class CreateSearchDataSourceResp(object):
    data_source: CreateSearchDataSourceRespDataSource = attr.ib(
        default=None, metadata={"req_type": "json"}
    )  # 数据源实例


def _gen_create_search_data_source_req(request, options) -> RawRequestReq:
    return RawRequestReq(
        dataclass=CreateSearchDataSourceResp,
        scope="Search",
